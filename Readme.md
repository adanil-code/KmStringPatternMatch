...
* **Kernel Logging & Persistent Telemetry Capture:** When executing in kernel mode, the test driver streams all diagnostic messages, test progression, and benchmark results in real time using `DbgPrintEx(DPFLTR_IHVDRIVER_ID, ...)` and automatically saves the complete telemetry output directly to the `\SystemRoot\Temp\KmStringPatternMatchPerf.txt` file.
...
* **`/TestKm/`**: Native kernel-mode test driver.
  * `KmStringPatternMatchDrv.cpp` & `.inf`: Deploys the test suite as a fully operational Windows driver for true bare-metal kernel performance validation. Outputs comprehensive test logs and performance metrics using `DbgPrintEx(DPFLTR_IHVDRIVER_ID, ...)` and simultaneously persists them to the `\SystemRoot\Temp\KmStringPatternMatchPerf.txt` file. Includes MSVC 2026 `.slnx` and `.vcxproj` files.
* **`/TestContainer/`**: 
